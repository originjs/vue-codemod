...
function nodeFilter(node: Node): boolean {
  // @ts-ignore
  const pre = (item: VAttribute) => item.key.name === 'bind' && item.key.argument?.name === 'format'
  return node.type === 'VAttribute'
    ...
    // @ts-ignore
    && node.value.expression.properties.filter(item => item.key.name === 'format').length > 0
...
//  get format attribute in the time-picker
  // @ts-ignore
  let formatValue = node.value.expression.properties.filter(item => item.key.name === 'format')[0].value.value
//  add format attribute to el-time-picker tag
  fixOperations.push(OperationUtil.insertTextBefore(node, `format='${formatValue}' `))

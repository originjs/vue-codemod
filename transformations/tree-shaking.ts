...
export const transformAST: ASTTransformation = context => {
  const beforeCount = Object.keys(subRules).reduce( (sum, key) => sum + subRules[key], 0)
  nextTick(context)
  observable(context)
  version(context)
  // remove import 'Vue' from 'vue' if not used
  removeImport(context, { localBinding: 'Vue' })
  const afterCount = Object.keys(subRules).reduce( (sum, key) => sum + subRules[key], 0)
  const change = afterCount - beforeCount
  const cntFunc = getCntFunc('tree-shaking', outputReport)
  cntFunc(change)
}
